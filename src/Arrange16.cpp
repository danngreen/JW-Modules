...
#define ARRANGE_COLS 16
#define CELLS 1024
#define CELLW 10
#define CELLH 20

struct Arrange16 : Module {
	enum ParamIds {
		...
		if (rowNamesJ && json_array_size(rowNamesJ) == ARRANGE_ROWS) {
			for (int i = 0; i < ARRANGE_ROWS; i++) {
				...
				if (rowNameJ) {
					...
					getOutputInfo(MAIN_OUTPUT + i)->description = rowNames[i];
					dirtyNames[i] = true;
				}
			}
		}
...
struct Arrange16Display : LightWidget {
	Arrange16 *module;
...
struct RndModeKnob2 : JwSmallSnapKnob {
	...
	std::string formatCurrentValue() override {
		if(getParamQuantity() != NULL){
			switch(int(getParamQuantity()->getDisplayValue())){
				case Arrange16::RND_BASIC:return "Basic";
				case Arrange16::RND_EUCLID:return "Euclid";
				case Arrange16::RND_SIN_WAVE:return "Sine";
				case Arrange16::RND_MIRROR_X:return "Mirror X";
				case Arrange16::RND_MIRROR_Y:return "Mirror Y";
			}
		}
		return "";
	}
...
struct RowTextField16 : LedDisplayTextField {
	...
	void onChange(const ChangeEvent& e) override {
		if (module) {
			...
			module->getOutputInfo(Arrange16::MAIN_OUTPUT + i)->description = getText();
        }
	}
};
...
struct Arrange16AbsolutePosModeItem : MenuItem {
	Arrange16 *arrange;
...
struct Arrange16DefaultWhenNoInputModeItem : MenuItem {
	Arrange16 *arrange;
	void onAction(const event::Action &e) override {
...
struct Arrange16Widget : ModuleWidget { 
	Arrange16Widget(Arrange16 *module); 
	void appendContextMenu(Menu *menu) override;
};

Arrange16Widget::Arrange16Widget(Arrange16 *module) {
	setModule(module);
	box.size = Vec(RACK_GRID_WIDTH*23, RACK_GRID_HEIGHT);
	...
	addInput(createInput<TinyPJ301MPort>(Vec(45, topPort), module, Arrange16::CLOCK_INPUT));
	addInput(createInput<TinyPJ301MPort>(Vec(75, topPort), module, Arrange16::RESET_INPUT));
	addParam(createParam<SmallButton>(Vec(95, topKnob), module, Arrange16::RESET_BTN_PARAM));

	addInput(createInput<TinyPJ301MPort>(Vec(140, topPort), module, Arrange16::START_INPUT));
	addParam(createParam<JwSmallSnapKnob>(Vec(160, topKnob), module, Arrange16::START_PARAM));
	addInput(createInput<TinyPJ301MPort>(Vec(205, topPort), module, Arrange16::LENGTH_INPUT));
	...
	addInput(createInput<TinyPJ301MPort>(Vec(270, 70), module, Arrange16::CLEAR_INPUT));
	addParam(createParam<SmallButton>(Vec(290, 65), module, Arrange16::CLEAR_BTN_PARAM));
	
	addInput(createInput<TinyPJ301MPort>(Vec(270, 128), module, Arrange16::RND_TRIG_INPUT));
	addParam(createParam<SmallButton>(Vec(290, 123), module, Arrange16::RND_TRIG_BTN_PARAM));

	addInput(createInput<TinyPJ301MPort>(Vec(270, 192), module, Arrange16::RND_AMT_INPUT));
	addParam(createParam<SmallWhiteKnob>(Vec(290, 187), module, Arrange16::RND_AMT_KNOB_PARAM));

	
	RndModeKnob2 *rndModeKnob2 = dynamic_cast<RndModeKnob2*>(createParam<RndModeKnob2>(Vec(279, 245), module, Arrange16::RND_MODE_KNOB_PARAM));
	CenteredLabel* const rndModeLabel = new CenteredLabel;
	rndModeLabel->box.pos = Vec(146, 120);
	rndModeLabel->text = "";
	rndModeKnob2->connectLabel(rndModeLabel, module);
	addChild(rndModeLabel);
	addParam(rndModeKnob2);

	addOutput(createOutput<TinyPJ301MPort>(Vec(253, 332), module, Arrange16::POS_OUTPUT));
	addOutput(createOutput<TinyPJ301MPort>(Vec(283, 332), module, Arrange16::INTENSITY_OUTPUT));
	addOutput(createOutput<TinyPJ301MPort>(Vec(313, 332), module, Arrange16::EOC_OUTPUT));
...
void Arrange16Widget::appendContextMenu(Menu *menu) {
	MenuLabel *spacerLabel = new MenuLabel();
	menu->addChild(spacerLabel);

	Arrange16 *arrange16 = dynamic_cast<Arrange16*>(module);

	Arrange16AbsolutePosModeItem *absolutePosMode = new Arrange16AbsolutePosModeItem();
	absolutePosMode->text = "Absolute Position";
	absolutePosMode->arrange = arrange16;
	menu->addChild(absolutePosMode);

	Arrange16DefaultWhenNoInputModeItem *def = new Arrange16DefaultWhenNoInputModeItem();
	def->text = "Default to 10V out when no input";
	def->arrange = arrange16;
	menu->addChild(def);
}

Model *modelArrange16 = createModel<Arrange16, Arrange16Widget>("Arrange16");
